...
angular.module('nuBoard')
  .service('KineticSmoothie', function () {

    /**
     * interpolate between two points
     *
     * @param {Number} index in transposed array that holds starting point
     * @param {Function} interpolation function
     */
    var smoothenCurveSegment = function(i, s) {
      ...
      var pieceCount = 2; // should be a power of two so the for loop comes out exact
      var smoothenedSegment = [];
      for (var t = 0, step = 1 / pieceCount; t < 1; t += step) {
        var start = s(i + t);
        var end = s(i + t + 1 / pieceCount);
        // get distance between start and end
        var pieceLength = Math.sqrt(Math.pow(start[0] - end[0], 2) + Math.pow(start[1] - end[1], 2));
        // compute du so that we get the desired average line length
        var du = averageLineLength / pieceLength;
        if (du < 0) console.log('!!! weird: du < 0', du);
        for (var u = 0; u < step; u += du) {
          var p = s(i + t + u);
          smoothenedSegment.push(p[0], p[1]);
        }
      }
      // ensure that the path actually passes through the end points
      var p = s(i + 1);
      smoothenedSegment.push(p[0], p[1]);
      return smoothenedSegment;
    };

    /**
     * interpolate list of points (line)
     *
     * @param {Array} flat array of alternating x/y coordinates
     */
    this.interpolate = function(points) {
      // transpose flat array to [[x0,y0],...,[xn,yn]]
      var points_transposed = [];
      _.forEach(points, function(value, index) {
        if (index % 2 === 0) points_transposed.push([value, points[index + 1]]);
      });
      if (points_transposed.length >= 2) {
        var s = Smooth(points_transposed, {method: "cubic", clip: "clamp", cubicTension: 0.4});
        var points_smoothened = [];
        _.forEach(points_transposed, function(point, index) {
          points_smoothened = points_smoothened.concat(smoothenCurveSegment(index, s));
        });
        return points_smoothened;
      } else {
        return points;
      }
    };
